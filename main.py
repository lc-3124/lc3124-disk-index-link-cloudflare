...
<body>
    ...
    <script>
        ...
        async function loadBroadcastContent() {{
            ...
            try {{
                contentElement.textContent = '正在加载公告...';
                // 发起请求读取broad.txt文件
                console.log('开始加载公告文件: broad.txt');
                const response = await fetch('broad.txt', {{
                    method: 'GET',
                    cache: 'no-cache'  // 禁用缓存，确保读取最新内容
                }});
                ...
                if (!response.ok) {{
                    ...
                    if (response.status === 404) {{
                        errorMsg = '找不到broad.txt文件，请检查文件是否存在';
                    }} else if (response.status === 403) {{
                        errorMsg = '没有权限读取broad.txt文件';
                    }} else {{
                        errorMsg = `加载失败 (状态码: ${{response.status}})`;
                    }}
                ...
                // 显示用户友好的错误信息 + 调试提示
                contentElement.textContent = `加载公告出错: ${{error.message}}
（请按F12打开开发者工具 → Console查看详细错误）`;
            }}
        }}
